...
public class PurchasedBike extends Bike {
    ...
    /******
     * This method is used to display the unique bike id, model name, final price (quantity * price per bike + quantity
     * * $50 if
    ...
    public String shoppingCartToString() {
        String message = String.format("Bike ID %d | Model Name %s | Price: %.2f | Wheel size: %d | Quantity: %d",
                getId(), getModelName(), getFinalPrice(), getWheelSize(), getQuantity());
        return message;
    }
}
